...
static err_t low_level_init(struct netif *netif)
{
    ...
    HAL_ETH_Start(&ETH_Handler); // start mac and dma
    return ERR_OK;
}

static err_t low_level_output(struct netif *netif, struct pbuf *p)  //发送数据
{
    err_t errval;
    struct pbuf *q;
...
static struct pbuf *low_level_input(struct netif *netif)    //获取接收数据
{
    struct pbuf *p = NULL;
    struct pbuf *q;
    uint16_t len;
...
err_t ethernetif_init(struct netif *netif)
{
    ...
    netif->output = etharp_output;        //发送数据
    netif->linkoutput = low_level_output;   //接收数据

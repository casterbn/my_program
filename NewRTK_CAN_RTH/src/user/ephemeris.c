                      ...
                      int opt, double *rs, double *dts, double *var, int *svh)
{
    ...
    *var=var_urassr(ssr->ura);
    ...
    trace(5,"satpos_ssr: %s sat=%3d deph=%6.3f %6.3f %6.3f er=%6.3f %6.3f %6.3f dclk=%6.3f var=%6.3f\n",
...
*-----------------------------------------------------------------------------*/
extern int8_t satposs(obs_t *obs, vec_t *vec, nav_t *nav, int ephopt)
{
    ...
    for (i = 0; i < obs->n; i++) {
		...
		if (sys == _SYS_GLO_) continue; /* do not use GLO NOW */
        ...
        if (!satpos(time[i], teph, obs->data[i].sat, ephopt, nav, vec[i].satpvt + 0, vec[i].satpvt + 6, &vec[i].var, &vec[i].svh))
        {
#ifdef _TRACE_
        ...
        if (vec[i].satpvt[6] == 0.0)
        {
            if (!ephclk(time[i], teph, obs->data[i].sat, nav, vec[i].satpvt + 6)) continue;
    ...
    for (i = 0; i < obs->n; i++)
    {
        if (vec[i].satpvt[0] == 0.0) continue;
		sys = satsys(obs->data[i].sat, &prn);
#ifdef _TRACE_
		trace(3, "%s sat=%c%02d rs=%13.3f %13.3f %13.3f dts=%12.3f var=%7.3f svh=%02X\n", time_str(time[i], 6), sys2char(sys), prn, vec[i].satpvt[0], vec[i].satpvt[1], vec[i].satpvt[2]
            , vec[i].satpvt[6] * 1E9, vec[i].var, vec[i].svh);
#endif
    }

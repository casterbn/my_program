...
int line = 0;
int first_read = 0;
extern unsigned char gps_buff[4096];
        ...
        //if(get_data)
		{
			...
			else
			{
				...
				else
				{
					...
					gps_data_next = gps_buff;
					stat = line_read(fp, 8, offset, &gps_data_next, &next_gps_len,&excess_len);         //读取下一包gps
					//printf("4444444444444444444 next_gps_len =  %d\n", next_gps_len);
#if 0
					printf("gps_data_next =  ");
					
					for (int i = 0; i < next_gps_len; i++)
					{
						printf(" %02x", gps_data_next[i]);
					}
					printf("\n");
#endif
...
#endif
						memcpy(cur_data.gps_data + cur_data_len, gps_data_next, next_gps_len);
					...
					cur_data.gps_data_len = cur_data_len + next_gps_len;
				}
			}
